...
void FSamplePointLightTask::Init(FCompileDefinitions* CompileDefinitions)
{
    auto& DescriptorSetManager = VK_CONTEXT()->DescriptorSetManager;
...
void FSamplePointLightTask::UpdateDescriptorSets()
{
    for (size_t i = 0; i < TotalSize; ++i)
    {
		...
		UpdateDescriptorSet(RAYTRACE_SAMPLE_POINT_LIGHT_LAYOUT_INDEX, RAYTRACE_SAMPLE_POINT_LIGHT_UNIFORM_BUFFER_INDEX, i, RESOURCE_ALLOCATOR()->GetBuffer(UTILITY_INFO_POINT_LIGHT_BUFFER));
        UpdateDescriptorSet(RAYTRACE_SAMPLE_POINT_LIGHT_LAYOUT_INDEX, RAYTRACE_SAMPLE_POINT_LIGHT_NORMAL_AOV_BUFFER_INDEX, i, RESOURCE_ALLOCATOR()->GetBuffer(NORMAL_AOV_BUFFER));
		UpdateDescriptorSet(RAYTRACE_SAMPLE_POINT_LIGHT_LAYOUT_INDEX, RAYTRACE_SAMPLE_POINT_LIGHT_WORLD_SPACE_POSITION_AOV_BUFFER_INDEX, i, RESOURCE_ALLOCATOR()->GetBuffer(WORLD_SPACE_POSITION_AOV_BUFFER));
        UpdateDescriptorSet(RAYTRACE_SAMPLE_POINT_LIGHT_LAYOUT_INDEX, RAYTRACE_SAMPLE_POINT_LIGHT_SAMPLED_LIGHT_BUFFER_INDEX, i, RESOURCE_ALLOCATOR()->GetBuffer(SAMPLED_POINT_LIGHT_BUFFER));
		UpdateDescriptorSet(RAYTRACE_SAMPLE_POINT_LIGHT_LAYOUT_INDEX, RAYTRACE_SAMPLE_POINT_LIGHT_PIXEL_INDEX_BUFFER, i, RESOURCE_ALLOCATOR()->GetBuffer(PIXEL_INDEX_BUFFER));
		UpdateDescriptorSet(RAYTRACE_SAMPLE_POINT_LIGHT_LAYOUT_INDEX, RAYTRACE_SAMPLE_POINT_LIGHT_RENDER_ITERATION_BUFFER_INDEX, i, RESOURCE_ALLOCATOR()->GetBuffer(RENDER_ITERATION_BUFFER));
    }
};
