        ...
        FExecutableTask(WidthIn, HeightIn, SubmitXIn, SubmitYIn, LogicalDevice)
{
	...
	DescriptorSetManager->AddDescriptorLayout(Name, MASTER_SHADER_LAYOUT_INDEX, MASTER_SHADER_IBL_PDF_BUFFER_INDEX,
		{VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,  VK_SHADER_STAGE_RAYGEN_BIT_KHR});
...
void FMasterShader::UpdateDescriptorSets()
{
    for (size_t i = 0; i < TotalSize; ++i)
    {
		...
		UpdateDescriptorSet(MASTER_SHADER_LAYOUT_INDEX, MASTER_SHADER_IBL_PDF_BUFFER_INDEX, i, RESOURCE_ALLOCATOR()->GetBuffer("IBLPDFBuffer"));
		...
		UpdateDescriptorSet(MASTER_SHADER_LAYOUT_INDEX, MASTER_SHADER_DEBUG_LAYER_IMAGE_INDEX, i, TEXTURE_MANAGER()->GetFramebufferImage("DebugLayerImage"));
    }
};
