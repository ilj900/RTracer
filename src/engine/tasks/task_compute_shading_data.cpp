...
void FComputeShadingDataTask::UpdateDescriptorSets()
{
    for (uint32_t i = 0; i < TotalSize; ++i)
    {
        ...
        UpdateDescriptorSet(COMPUTE_SHADING_DATA_LAYOUT_INDEX, COMPUTE_SHADING_DATA_HITS_BUFFER_INDEX, i, RESOURCE_ALLOCATOR()->GetBuffer(HITS_BUFFER));
        UpdateDescriptorSet(COMPUTE_SHADING_DATA_LAYOUT_INDEX, COMPUTE_SHADING_DATA_RAYS_BUFFER_INDEX, i, RESOURCE_ALLOCATOR()->GetBuffer(INITIAL_RAYS_BUFFER));
		UpdateDescriptorSet(COMPUTE_SHADING_DATA_LAYOUT_INDEX, COMPUTE_SHADING_DATA_TRANSFORM_INDEX_BUFFER_INDEX, i, RESOURCE_ALLOCATOR()->GetBuffer(TRANSFORM_INDEX_BUFFER));
		UpdateDescriptorSet(COMPUTE_SHADING_DATA_LAYOUT_INDEX, COMPUTE_SHADING_DATA_RAY_INDEX_MAP_INDEX, i, RESOURCE_ALLOCATOR()->GetBuffer(PIXEL_INDEX_BUFFER));
        UpdateDescriptorSet(COMPUTE_SHADING_DATA_LAYOUT_INDEX, COMPUTE_SHADING_DATA_NORMAL_AOV_DATA_INDEX, i, RESOURCE_ALLOCATOR()->GetBuffer(NORMAL_AOV_BUFFER));
        UpdateDescriptorSet(COMPUTE_SHADING_DATA_LAYOUT_INDEX, COMPUTE_SHADING_DATA_UV_AOV_DATA_INDEX, i, RESOURCE_ALLOCATOR()->GetBuffer(UV_AOV_BUFFER));
        UpdateDescriptorSet(COMPUTE_SHADING_DATA_LAYOUT_INDEX, COMPUTE_SHADING_DATA_WORLD_SPACE_POSITION_AOV_DATA_INDEX, i, RESOURCE_ALLOCATOR()->GetBuffer(WORLD_SPACE_POSITION_AOV_BUFFER));
    }
};
...
void FComputeShadingDataTask::RecordCommands()
{
	...
	auto ActiveRayCountBufferDeviceAddress = RESOURCE_ALLOCATOR()->GetBuffer(ACTIVE_RAY_COUNT_BUFFER);
